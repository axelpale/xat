...
const summarize = (year, salesData, rewardsData, airdropsData) => {
  ...
  const rewards = rewardsData.reduce((acc, assetReward) => {
    return acc.plus(assetReward.rewardIncomeEur)
  }, ZERO)
  ...
  const otherIncomeTotal = rewards.plus(airdrops)
  ...
  return {
    ...
    otherCapitalIncomeTotalEur: otherIncomeTotal,
...
module.exports = function (accounts, events) {
  ...
  // Translate the keys
  const localizedSummaryRows = summaryRows.map(row => tok(row))

  if (localizedSummaryRows.length > 0) {
    const rangeStr = `${yearRange.minYear}-${yearRange.maxYear}`
    const reportLabel = t('incomeSummaryReport')
    const filename = reportLabel + '_' + rangeStr + '.csv'
    const filepath = path.join(OUTPUT_DIR, filename)
    arrayToCsv(localizedSummaryRows, filepath, 2)
  }
}
