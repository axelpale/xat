...
proto.getBalanceOfAcquisition = function (acquisitionId) {
  // Compute how much the account holds assets from
  ...
  for (i = 0; i < len; i++) {
    asset = this.assets[i]
    if (asset.acquisitionId === acquisitionId) {
      amountSum = amountSum.plus(asset.amount)
    }
  }
