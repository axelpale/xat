...
module.exports = function (row, accounts, events) {
  ...
  // Record an internal transfer event.
  const recordedAssets = sentAssets.map(x => x.copy())
  // Record how much of the same acquisition was left on the source account.
  recordedAssets.forEach(asset => {
    const acqId = asset.acquisitionId
    const acqSourceBalance = sourceAccount.getBalanceOfAcquisition(acqId)
    asset.acquisitionRemainingInSource = acqSourceBalance
  })
  const tev = new Transfer(
    row.id,
    row.date,
    recordedAssets,
