...
module.exports = function (accounts, events) {
  ...
  for (let y = yearRange.minYear; y <= yearRange.maxYear; y++) {
    const salesData = collectSalesData(accounts, events, y)
    const rewardsData = collectRewardsData(accounts, events, y)

    const summaryRow = summarize(y, salesData, rewardsData)
