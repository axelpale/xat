...
const getDateFromTime = require('./getDateFromTime')
...
module.exports = function (rows) {
  ...
  rows.forEach(row => {
    if (rewardTypes.includes(row.type)) {
      ...
      if (!accountName) {
        throw new Error('Unexpected empty toAccount name')
      }
      finalRewards[accountName] = row
    } else {
  ...
  resultRows.forEach(row => {
    if (row.rewardRangeBegin && row.rewardRangeEnd) {
      ...
      const beginDate = getDateFromTime(row.rewardRangeBegin)
      const endDate = getDateFromTime(row.rewardRangeEnd)
