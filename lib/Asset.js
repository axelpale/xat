...
const Asset = function (
  ...
  if (documents.some(d => typeof d !== 'string')) {
    throw new Error('Invalid docuemnts array. Must be an array of strings.')
  }
  if (typeof acquisitionId !== 'number') {
    throw new Error('Invalid acquisitionId. Must be an integer.')
  }
  if (acquisitionId !== parseInt(acquisitionId)) {
    throw new Error('Unexpected acquisitionId. Must be an integer.')
  }
  ...
  // Documentation for the purchase and expenses. Copy for side-effect safety.
...
proto.addExpenseEur = function (date, amount) {
  ...
  if (ageInDays < 0) {
    throw new Error('Unexpected expense date: ' +
      'cannot be earlier than the acquisition of the asset.')
  }
...
// ANTIPATTERN proto.getAcquisitionExpensesEur
// Acquisition expenses cannot be computed with certainty until
// the asset is disposed. Otherwise they might become counted twice
// with sale expenses. There is also special handling regarding gift
// acquisition costs and acquisition cost assumptions.
