...
const createLedgerRow = function (raw) {
...
module.exports = async function (filepath) {
  // Read valid ledger rows from the file.
  // Skip empty lines. Maintain the row order.
  // Normalize column names to camelCase properties.
  //
  // Return
  //   a Promise, resolve an array of normalized ledger rows.
  //
    ...
    fs.createReadStream(filepath)
      ...
      .on('data', (rawrow) => {
        ...
        if (isValidRawRow(rawrow)) {
          ...
          const ledgerRow = createLedgerRow(rawrow)
          // Collect
          rows.push(ledgerRow)
        } else {
          numInvalid += 1
      ...
      .on('end', () => {
        console.log('Ledger CSV file successfully read.')
      ...
      .on('error', (err) => {
        console.error('Error reading ledger CSV:', err)
