...
module.exports = function (accounts, events, beginDate, endDate) {
  // Print a readable proof-of-stake rewards report for the given date range.
  ...
  //
  const reportData = collectRewardsData(accounts, events, beginDate, endDate)

  if (reportData.length > 0) {
    const rangeLabel = beginDate + '_' + endDate
    const filename = 'rewards_report_' + rangeLabel + '.csv'
