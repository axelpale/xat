...
const GiftOut = require('../events/GiftOut')
...
module.exports = function (row, accounts, events) {
  ...
  const givenAssets = account.popAssets(amount)

  // Record a gift giving event
  const recordedAssets = givenAssets.map(a => a.copy())
  const ev = new GiftOut(
    row.id,
    row.date,
    row.documents,
    row.toAccount,
