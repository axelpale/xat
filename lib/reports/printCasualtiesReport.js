...
const collectCasualtiesData = require('./collectCasualtiesData')

const OUTPUT_DIR = path.resolve(__dirname, '..', '..', 'reports')

module.exports = function (accounts, events, beginDate, endDate) {
  // Print a readable report of asset casualties for the given date range.
  ...
  //
  const reportData = collectCasualtiesData(
    accounts,
    events,
    beginDate,
    endDate
  ...
  if (reportData.length > 0) {
    ...
    const filename = 'casualties_' + rangeLabel + '.csv'
