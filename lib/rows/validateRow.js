...
const MAX_FEE_RATIO_THRESHOLDS = {
  ...
  reward: new BigNumber('0.01'),
  payment_out: new BigNumber('3.00')
}

const prettyRange = (arr, prec) => {
  const minmax = findRange(arr)
  const minstr = minmax.min.toFixed(prec)
  const maxstr = minmax.max.toFixed(prec)
...
module.exports = function (row) {
  ...
  if (row.date[4] !== '-' || row.date[7] !== '-') {
    throw new Error('Invalid row date separator: ' + row.date)
  }
  ...
  // Check that if amounts are given, unit prices are given.
  if (row.sentAmount) {
    if (!(row.sentUnitPriceEur instanceof BigNumber)) {
      const msg = 'Missing or invalid unit price ' +
        'for sent unit: ' + row.sentUnit
      throw new Error(msg)
    }
  }
  if (row.receivedAmount) {
    if (!(row.receivedUnitPriceEur instanceof BigNumber)) {
      const msg = 'Missing or invalid unit price ' +
        'for received unit: ' + row.receivedUnit
      throw new Error(msg)
    }
  }
  if (row.feeAmount) {
  ...
  if (row.sentUnit && row.sentUnitPriceEur) {
    ...
    const apiPrices = [
      prices.getPriceEur(row.sentUnit, yesterday(row.date)),
      prices.getPriceEur(row.sentUnit, row.date),
      prices.getPriceEur(row.sentUnit, tomorrow(row.date))
    ].filter(x => x !== null)

    if (apiPrices.length > 0) {
      const somePricesMatch = apiPrices.some(apiUnitPrice => {
        return inSpread(ledgerUnitPrice, apiUnitPrice, UNIT_PRICE_TOLERANCE)
      })
      if (!somePricesMatch) {
        throw new Error('Sent unit price differs from ' +
          'the historical ' + row.sentUnit + ' price too much: ' +
          ledgerUnitPrice.toFixed(2) + ' EUR vs ' +
          prettyRange(apiPrices, 2) + ' EUR.')
      }
    }
  }
  ...
  if (row.receivedUnit && row.receivedUnitPriceEur) {
    ...
    const apiPrices = [
      prices.getPriceEur(row.receivedUnit, yesterday(row.date)),
      prices.getPriceEur(row.receivedUnit, row.date),
      prices.getPriceEur(row.receivedUnit, tomorrow(row.date))
    ].filter(x => x !== null)
