...
module.exports = function (accounts, events, beginDate, endDate) {
  // Print a readable report of received airdrops for the given date range.
  ...
  //
  const reportData = collectAirdropsData(accounts, events, beginDate, endDate)

  if (reportData.length > 0) {
    const rangeLabel = beginDate + '_' + endDate
    const filename = 'airdrops_report_' + rangeLabel + '.csv'
