...
const normalizeRawRow = function (raw) {
  ...
  let documents = ('' + raw.Documents).trim()
  ...
  let sentUnitPriceEur = parseNumber(raw['Sent Unit Price EUR'], null)
  const receivedAmount = parseNumber(raw.Received, null)
  const receivedUnit = ('' + raw['Received Unit']).trim().toUpperCase()
  let receivedUnitPriceEur = parseNumber(raw['Received Unit Price EUR'], null)
  ...
  // Convert document references to array.
  ...
  if (feeUnitPriceEur === null) {
    ...
    } else if (feeAmount.eq(ZERO)) {
      feeUnitPriceEur = ZERO
    }
  }
  ...
  return {
    id: 0, // to be set after normalization
    date,
    documents,
    type,
    desc,
...
module.exports = async function (filepath) {
  ...
  // Skip empty lines. Maintain order but ensure the oldest are first.
  ...
  // Get filename for readable console output.
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filepath)
      ...
      .on('end', () => {
        console.log('Transaction journal (' + basename + ') ' +
          'was read successfully:')
        console.log('  ' + rows.length + ' transactions found.')
        console.log('  ' + numInvalid + ' empty or invalid rows skipped.')
        console.log()

        // Consistent chronological order: oldest first
        const firstDate = rows[0].date
        const lastDate = rows[rows.length - 1].date
        if (lastDate < firstDate) {
          // Reverse in place so that oldest first
          rows.reverse()
        }

        // Skip the info rows that are not meant for computing.
        const materialRows = rows.filter(r => r.type !== 'info')

        // Give each row a reference id number.
        materialRows.forEach((row, i) => {
          row.id = i
        })

        // Resolve
        resolve(materialRows)
      })
      .on('error', (err) => {
