...
module.exports = function (row, accounts, events) {
  ...
  if (feeUnit === sourceUnit && feeAmount.gt(0) && feeUnit !== 'EUR') {
    ...
    const feeExpenses = new BigNumber(0)
    const feeSale = new Sale(row.date, feeAssets, feePriceEur, feeExpenses)
  ...
  if (sourceUnit !== 'EUR') {
    const salePrice = sentAmount.times(row.sentUnitPriceEur)
